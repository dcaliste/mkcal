...
public:
    ...
    */
    typedef QMultiHash<QString, KCalendarCore::Incidence*> Collection;

    /**
      A list of incidences, indexed by notebook ids.
    */
    typedef QMultiHash<QString, KCalendarCore::Incidence::Ptr> SharedCollection;

    /**
      A list of notebooks.
    */
    typedef QVector<Notebook*> Library;
    ...
    */
    ...
    /**
      Execute the given changes in the database. The provided collections
      are deleted are the call.
    ...
    /**
      ...
      @return True on success, false otherwise.
     */
    virtual bool storeIncidences(const SharedCollection &additions,
    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
          Notify the Manager that notebooks have been loaded from storage.
          The manager can then take ownership of the data.

          @param storage is a pointer to the StorageBackend object that
          is being observed.
          @param notebooks is a list of notebooks.
        */
        virtual void newNotebooks(StorageBackend *storage,
                                  const Library &notebooks) = 0;

        /**
          Notify the Manager that incidences have been loaded from storage.
          The manager can then take ownership of the data.
        ...
        */
        virtual void newIncidences(StorageBackend *storage,
                                   const Collection &incidences) = 0;
    };

    /**
      Registers a memory Manager for this Storage. Only one manager
      can be registered at a time.

      @param manager is a pointer to a Manager object that will take
      ownership of the objects generated by this Storage.

      @see unregisterManager()
     */
    void registerManager(Manager *manager);

    /**
      Unregisters the memory Manager of this Storage.
    ...
    public:
        ...
        */
        ...
        /**
          ...
          StorageBackend::storeIncidences() for instance.
    ...
    */
    static Notebook* createDefaultNotebook(QString name = QString(),
                                           QString color = QString());

protected:
    virtual bool modifyNotebook(const Notebook &nb, DBOperation dbop,
                                bool signal = true) = 0;
    virtual bool modifyIncidences(const Collection &additions,
                                  const Collection &modifications,
                                  const Collection &deletions,
    ...
    void storageUpdated(const Collection &added,
                        const Collection &modified,
                        const Collection &deleted);
    void newIncidences(const Collection &incidences);
    void newNotebooks(const Library &notebooks);
